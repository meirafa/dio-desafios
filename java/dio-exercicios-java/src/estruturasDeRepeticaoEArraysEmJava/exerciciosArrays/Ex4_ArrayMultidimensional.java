package estruturasDeRepeticaoEArraysEmJava.exerciciosArrays;
...
public class Ex4_ArrayMultidimensional {
    public static void main(String[] args) {
        ...
        //variáveis
        int maior = Integer.MAX_VALUE; //variável que guarda o maior valor
        ...
        for (int i = 0; i < M.length; i++) { //navegando na matriz
            ...
            for (int j = 0; j < M[i].length; j++) { //navegando na linha (array unidimensional)
                ...
                //se o elemento em uma determinada posição (linha e coluna) é menor que o maior número
                if (M[i][j] < maior) {

                    maior = M[i][j]; //o valor da variável menor passa a ser o valor aleatório
                    linha = i; //que está na linha representada pela variável i
                    coluna = j; //que esta na coluna representada pela variável j
                }
            }
        }
        System.out.println("Menor: " + maior); //menor valor
